...
class go_board:
    ...
    def copy_game(self):
        ...
        # Copy array of lists
        copy_object.allies = cPickle.loads(cPickle.dumps(self.allies, -1))
        copy_object.liberties = cPickle.loads(cPickle.dumps(self.liberties, -1))
    ...
    def check_capture(self, index, played_color_number, played):
        ...
        if (self.liberties[index][0] < 1) & (self.board[index]!=played_color_number):
    ...
    def can_capture_black(self, position, color_number):
        ...
        """
        neighbours = self.neighbours[position]
        indexes = tuple(zip(*neighbours))
        liberties = self.liberties[indexes]==1
        colors = self.board[indexes]==1
        
        result = sum(np.logical_xor(liberties,colors))
        return (result!=0,result!=len(neighbours))
       
        """
        neighbours = self.neighbours[position]
        
        liberty_list = list(map(self.get_liberty_and_color,neighbours))
    ...
    def get_legal_board(self, color):
        ...
        if color_number==1:
            capture_status = list(map(lambda position: self.can_capture_black(position, color_number),self.zero_liberties))
        ...
        # Check white legal moves
            ...
            capture_status = list(map(lambda position: self.can_capture_white(position, color_number),self.zero_liberties))
            for position, capture in zip(self.zero_liberties,capture_status):
    ...
    def map_area(self,point_board, board_size, starting_position, area, black_enc, white_enc):
        ...
        if (point_board[starting_position] == 3):
            ...
            for neighbour in neighbours:
                point_board, area, black_enc, white_enc = self.map_area(point_board, board_size, neighbour, area, black_enc, white_enc)
        ...
        for i in range(self.board_size):
            for j in range(self.board_size):
                if (point_board[i,j]==3):
                    ...
                    black_enc = 0
                    white_enc = 0
                    point_board , area, black_enc, white_enc = self.map_area(point_board, self.board_size, (i,j), area, black_enc, white_enc)
